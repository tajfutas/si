...
class RegisterInstructions(type):
  ...
  # es
    def __init__(cls, name, bases, nmspc):
        ...
        bases_keys = {c.CMD for c in bases
            if c.CMD in cls._instrs}
        for k in bases_keys:
...
class Instruction(metaclass=RegisterInstructions):
...
class RecordOverflow(Exception):
  ...
  def __init__(self, instr, data):
    super.__init__('intruction data (self.instr) overflow '
        'with data (self.data)')
    self.instr = instr
    self.data = data
